...
# 聚簇索引 （索引和数据放到一块的，找到索引就找到了数据）
- 聚簇索引决定了表中数据的物理存储顺序。数据行的顺序与索引的顺序相同。
- 每个表只能有一个聚簇索引，因为数据行只能按一种顺序排列。
聚簇索引适合用于范围查询，例如查找一系列连续的值，因为数据是顺序存储的。
在创建主键时，通常会自动创建聚簇索引（如果没有指定其他索引）。

# 非聚簇索引（索引和数据是分开的 ，需要回表）
- 非聚簇索引与数据表的物理存储顺序无关。它包含索引值和指向数据行的指针。
- 一个表可以有多个非聚簇索引，每个索引可以在不同的列上。
可以创建多个非聚簇索引以支持不同的查询需求，这使得它们在复杂查询中非常有用。